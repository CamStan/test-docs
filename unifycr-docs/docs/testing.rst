...
sysio_suite in `t/Makefile.am`_ and `t/sys/sysio_suite.c`_::
    ...
    - Create a <test_suite_name>.c file (i.e., *sysio_suite.c*) that will
      ...
      - Maintain the number of TAP tests in this suite with ``plan(N)``
    ...
    - Create <test_name>.c files (i.e., *open.c*) that contains the testing
      ...
      - Update the number of tests in this suite in ``plan(N)``
...
To add new test cases to any existing suite of tests:
    ...
    1. Add the desired tests (order matters) to the appropriate <test_name>.c
       file
    2. Adjust the number, N, in ``plan(N)`` within the <test_suite_name>.c file
       to account for any added or removed tests
...
in (i.e., testing a wrapper that doesn't have any tests yet):
    ...
    4. Adjust the number, N, in ``plan(N)`` within the <test_suite_name>.c file
       to account for the added test cases
    5. The <function_name>_test function can now be called from the
       <test_suite_name>.c file
